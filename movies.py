...
del new_data['MPAATrue']

new_data['Budget'] = new_data['Budget'].apply(
    lambda x: int(re.sub(r'[$,]', '', x)) if (type(x) == str) and (re.search(r'[$]', x) != None) else None)
new_data = new_data[pd.notna(new_data['Budget'])]

new_data['Gross'] = new_data['Gross'].apply(
    lambda x: int(re.sub(r'[$,]', '', x)) if (type(x) == str) and (re.search(r'[$]', x) != None) else None)

new_data['TotalGross'] = new_data['TotalGross'].apply(
    lambda x: int(re.sub(r'[$,]', '', x)) if (type(x) == str) and (re.search(r'[$]', x) != None) else None)

new_data['Theaters'] = new_data['Theaters'].apply(
    lambda x: None if x == '-' else int(re.sub(r'[,]', '', x)))
new_data = new_data[pd.notna(new_data['Theaters'])]

sum_gross = new_data.groupby('Release').sum()
single_data = new_data.drop_duplicates('Release')
single_data = single_data.set_index('Release')
single_data[['Gross', 'TotalGross']] = sum_gross[['Gross', 'TotalGross']]
# %%
single_data.to_csv('data/cleaned_data.csv')
# %%
